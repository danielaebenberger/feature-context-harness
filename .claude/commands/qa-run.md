---
description: QA Harness — full six-stage orchestrator. Runs Acceptance Criteria, Test Adequacy, Persona UAT, and Documentation Review, then assembles the final release recommendation with mandatory human checkpoints.
argument-hint: [--pillars A,B,C,D] [--mode refine|validate] [--feature <slug>] [--skip-doc] [--resume stageN]
---
...
> **Claude Code note**: this command orchestrates all six stages inline, in this
> conversation — it does not shell out to the other `/qa-*` commands programmatically.
> Each stage below carries the same instructions as its standalone command
> (`/qa-ac-validate`, `/qa-cypress-analyze`, `/qa-persona-uat`, `/qa-doc-review`,
> `/qa-report`); follow them directly rather than trying to invoke another slash
> command mid-run.
...
**Run**: `/qa-persona-uat` (Steps 1–3 only: select → generate → checkpoint)
...
node harness/sensors/cypress-analyzer/cypress-analyzer.js \
  ...
  --verbose
```
Then apply `/qa-cypress-analyze` for semantic evaluation.
